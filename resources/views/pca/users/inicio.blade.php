        ...
        <tbody>
          @foreach ($modelo as $mod)
            <tr>
              ...
              <td>
                <a href="{{route('users.edit', ['id' => $mod->id])}}" class="btn btn-success m-btn m-btn--icon m-btn--icon-only"><i class="flaticon-edit-1"></i></a>
              </td>
              <td><form action="{{route('users.destroy', ['id' => $mod->id])}}" method="post">{{ csrf_field() }}{{ method_field('delete') }}<button type="submit" class="btn btn-danger m-btn m-btn--icon m-btn--icon-only"><i class="flaticon-circle"></i></button></form></td>
